...
const Messages = () => {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("clientPrefillMessageRoom");
    if (!raw || mergedRooms.length === 0) return;
    try {
      const wanted = JSON.parse(raw);
      if (!wanted?.eventId || !wanted?.otherUserId) return;
      const room = mergedRooms.find(
        (r) =>
          String(r.eventId) === String(wanted.eventId) &&
          String(r.otherUserId) === String(wanted.otherUserId)
      );
      if (room) {
        setActiveRoom(room);
      ...
      localStorage.removeItem("clientPrefillMessageRoom");
    }
  }, [mergedRooms]);
